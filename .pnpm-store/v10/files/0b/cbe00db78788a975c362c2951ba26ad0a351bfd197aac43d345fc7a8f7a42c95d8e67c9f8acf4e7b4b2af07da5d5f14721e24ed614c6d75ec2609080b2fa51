...
import { c as Pt, d as Wt, n as logger, s as Nt } from "./logger-O9Su9xI5.mjs";
import { r as relativeToProcess, t as loadKit } from "./kit-mAWtuYKF.mjs";
import { c as resolve, n as dirname, r as extname } from "./pathe.M-eThtNZ-C6qzirxH.mjs";
import { r as pascalCase, t as camelCase } from "./dist-BnlgscvS.mjs";
...
var add_default = defineCommand({
	...
	async run(ctx) {
		...
		Pt(colors.cyan("Adding template..."));
		...
		if (!name) {
			Nt("name argument is missing!");
			process.exit(1);
		}
		const config = await (await loadKit(cwd$1)).loadNuxtConfig({ cwd: cwd$1 });
