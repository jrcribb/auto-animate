import { t as __commonJSMin } from "./chunk-CuftcezF.mjs";
import { a as resolveModulePath, n as tryResolveNuxt } from "./kit-mAWtuYKF.mjs";
import { r as readPackageJSON } from "./dist-z8aedaRt.mjs";
import { a as require_lte, c as require_eq, d as require_compare, f as require_semver$1, h as require_constants, i as require_cmp, l as require_lt, m as require_re, n as require_comparator, o as require_gte, p as require_identifiers, r as require_range, s as require_neq, t as require_satisfies, u as require_gt } from "./satisfies-C79zCZRN.mjs";
...
var require_parse = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	const parse$6 = (version, options, throwErrors = false) => {
		if (version instanceof SemVer$11) return version;
		try {
			return new SemVer$11(version, options);
		} catch (er) {
...
var require_inc = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	const inc$1 = (version, release, options, identifier, identifierBase) => {
		...
		try {
			return new SemVer$10(version instanceof SemVer$10 ? version.version : version, options).inc(release, identifier, identifierBase).version;
		} catch (er) {
			return null;
		}
	};
...
var require_subset = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	const simpleSubset = (sub, dom, options) => {
		...
		let gt$3, lt$2;
		for (const c of sub) if (c.operator === ">" || c.operator === ">=") gt$3 = higherGT(gt$3, c, options);
		else if (c.operator === "<" || c.operator === "<=") lt$2 = lowerLT(lt$2, c, options);
		else eqSet.add(c.semver);
		if (eqSet.size > 1) return null;
		let gtltComp;
		if (gt$3 && lt$2) {
			gtltComp = compare$1(gt$3.semver, lt$2.semver, options);
			if (gtltComp > 0) return null;
			else if (gtltComp === 0 && (gt$3.operator !== ">=" || lt$2.operator !== "<=")) return null;
		}
		for (const eq$1 of eqSet) {
			if (gt$3 && !satisfies$1(eq$1, String(gt$3), options)) return null;
			if (lt$2 && !satisfies$1(eq$1, String(lt$2), options)) return null;
			for (const c of dom) if (!satisfies$1(eq$1, String(c), options)) return false;
			return true;
		}
		let higher, lower;
		let hasDomLT, hasDomGT;
		let needDomLTPre = lt$2 && !options.includePrerelease && lt$2.semver.prerelease.length ? lt$2.semver : false;
		let needDomGTPre = gt$3 && !options.includePrerelease && gt$3.semver.prerelease.length ? gt$3.semver : false;
		if (needDomLTPre && needDomLTPre.prerelease.length === 1 && lt$2.operator === "<" && needDomLTPre.prerelease[0] === 0) needDomLTPre = false;
		for (const c of dom) {
			...
			if (gt$3) {
				...
				if (c.operator === ">" || c.operator === ">=") {
					higher = higherGT(gt$3, c, options);
					if (higher === c && higher !== gt$3) return false;
				} else if (gt$3.operator === ">=" && !satisfies$1(gt$3.semver, String(c), options)) return false;
			}
			if (lt$2) {
				if (needDomLTPre) {
				...
				if (c.operator === "<" || c.operator === "<=") {
					lower = lowerLT(lt$2, c, options);
					if (lower === c && lower !== lt$2) return false;
				} else if (lt$2.operator === "<=" && !satisfies$1(lt$2.semver, String(c), options)) return false;
			}
			if (!c.operator && (lt$2 || gt$3) && gtltComp !== 0) return false;
		}
		if (gt$3 && hasDomLT && !lt$2 && gtltComp !== 0) return false;
		if (lt$2 && hasDomGT && !gt$3 && gtltComp !== 0) return false;
