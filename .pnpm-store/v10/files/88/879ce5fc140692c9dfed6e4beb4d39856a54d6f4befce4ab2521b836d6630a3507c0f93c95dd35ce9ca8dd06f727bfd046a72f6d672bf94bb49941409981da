...
//#region ../../node_modules/.pnpm/@parcel+watcher-wasm@2.5.1/node_modules/@parcel/watcher-wasm/node_modules/napi-wasm/index.mjs
const NAPI_OK = 0;
...
var Environment = class {
	...
	constructor(instance) {
		...
		let exports$1 = values.length;
		values.push(this.exports);
		try {
			if (this.instance.exports.napi_register_module_v1) this.instance.exports.napi_register_module_v1(this.id, exports$1);
			if (this.instance.exports.napi_register_wasm_v1) this.instance.exports.napi_register_wasm_v1(this.id, exports$1);
		} finally {
			this.popScope();
	...
	getBufferInfo(buf, ptr) {
		if (this.buffers.has(buf)) {
			let b$1 = this.buffers.get(buf);
			this.setPointer(ptr, b$1.byteOffset);
			return b$1.byteLength;
		}
		if (buf instanceof ArrayBuffer) {
			let b$1 = this.copyBuffer(new Uint8Array(buf));
			this.setPointer(ptr, b$1.byteOffset);
			return b$1.byteLength;
		}
	...
	createFunction(cb, data) {
		let env$2 = this;
		let fn = env$2.table.get(cb);
		let func = function(...args) {
			let scope = env$2.pushScope();
			try {
				let values = env$2.scopes[scope];
				...
				let res = fn(env$2.id, info);
				return env$2.get(res);
			} finally {
				env$2.popScope();
				if (env$2.pendingException) {
					let e = env$2.pendingException;
					env$2.pendingException = null;
					throw e;
				}
			}
		};
...
var FinalizeRecord = class {
	constructor(env$2, finalize, hint, data) {
		this.env = env$2;
...
var ThreadSafeFunction = class {
	constructor(env$2, fn, nativeFn, context) {
		this.env = env$2;
		this.fn = fn;
...
var AsyncWork = class {
	constructor(env$2, execute, complete, data) {
		this.env = env$2;
...
const napi = {
	napi_open_handle_scope(env_id, result) {
		let env$2 = environments[env_id];
		let id = env$2.pushScope();
		return env$2.setPointer(result, id);
	},
	napi_close_handle_scope(env_id, scope) {
		let env$2 = environments[env_id];
		if (scope !== env$2.scopes.length - 1) return NAPI_HANDLE_SCOPE_MISMATCH;
		env$2.popScope();
		return NAPI_OK;
	},
	...
	napi_escape_handle(env_id, scope_id, escapee, result) {
		let env$2 = environments[env_id];
		let value = env$2.get(escapee);
		return env$2.createValue(value, result, scope_id - 1);
	},
	...
	napi_set_property(env_id, object, key, value) {
		let env$2 = environments[env_id];
		let obj = env$2.get(object);
		let name = env$2.get(key);
		obj[name] = env$2.get(value);
		return NAPI_OK;
	},
	napi_get_property(env_id, object, key, result) {
		...
		let obj = env$2.get(object);
		let name = env$2.get(key);
		return env$2.createValue(obj[name], result);
	},
	napi_delete_property(env_id, object, key, result) {
		...
		if (result) env$2.memory[result] = res ? 1 : 0;
		return NAPI_OK;
	},
	napi_has_property(env_id, object, key, result) {
		...
		let obj = env$2.get(object);
		let name = env$2.get(key);
		env$2.memory[result] = name in obj ? 1 : 0;
		return NAPI_OK;
	},
	napi_has_own_property(env_id, object, key, result) {
		...
		let obj = env$2.get(object);
		let name = env$2.get(key);
		env$2.memory[result] = obj.hasOwnProperty(name) ? 1 : 0;
		return NAPI_OK;
	},
	napi_set_named_property(env_id, object, utf8Name, value) {
		let env$2 = environments[env_id];
		let obj = env$2.get(object);
		let val = env$2.get(value);
		let name = env$2.getString(utf8Name);
		obj[name] = val;
		return NAPI_OK;
	},
	...
	napi_get_property_names(env_id, object, result) {
		...
		return env$2.createValue(properties, result);
	},
	...
	napi_define_properties(env_id, object, property_count, properties) {
		...
		for (let i = 0; i < property_count; i++) {
			let descriptor = env$2.readPropertyDescriptor(ptr);
	...
	napi_get_prototype(env_id, object, result) {
		let env$2 = environments[env_id];
		let obj = env$2.get(object);
		return env$2.createValue(Object.getPrototypeOf(obj), result);
	},
	napi_define_class(env_id, utf8name, length, constructor, data, property_count, properties, result) {
		let env$2 = environments[env_id];
		let func = env$2.createFunction(constructor, data);
		Object.defineProperty(func, "name", {
			value: env$2.getString(utf8name, length),
			configurable: true
		});
		let ptr = properties >> 2;
		for (let i = 0; i < property_count; i++) {
		...
		return env$2.createValue(func, result);
	},
	napi_create_reference(env_id, value, refcount, result) {
		let env$2 = environments[env_id];
		let id = env$2.referenceId++;
		env$2.references.set(id, {
			value: env$2.get(value),
			refcount
		});
		return env$2.setPointer(result, id);
	},
	...
	napi_get_reference_value(env_id, ref, result) {
		let env$2 = environments[env_id];
		let reference = env$2.references.get(ref);
		return env$2.createValue(reference.value, result);
	},
	napi_reference_ref(env_id, ref, result) {
		let env$2 = environments[env_id];
		let reference = env$2.references.get(ref);
		reference.refcount++;
		return env$2.setPointer(result, reference.refcount);
	},
	...
	napi_set_instance_data(env_id, data, finalize_cb, finalize_hint) {
		let env$2 = environments[env_id];
		env$2.instanceData = data;
		return NAPI_OK;
	},
	napi_get_instance_data(env_id, data) {
		let env$2 = environments[env_id];
		return env$2.setPointer(data, env$2.instanceData);
	},
	...
	napi_get_value_uint32(env_id, value, result) {
		let env$2 = environments[env_id];
		let val = env$2.get(value);
		return env$2.setPointer(result, val);
	},
	...
	napi_get_value_bigint_int64(env_id, value, result, lossless) {
		...
		let val = env$2.get(value);
		env$2.i64[result >> 3] = val;
		if (lossless) env$2.memory[lossless] = BigInt.asIntN(64, val) === val ? 1 : 0;
		return NAPI_OK;
	},
	...
	napi_get_value_bigint_uint64(env_id, value, result, lossless) {
		let env$2 = environments[env_id];
		let val = env$2.get(value);
		env$2.u64[result >> 3] = val;
		if (lossless) env$2.memory[lossless] = BigInt.asUintN(64, val) === val ? 1 : 0;
		return NAPI_OK;
	},
	napi_create_bigint_words(env_id, sign_bit, word_count, words, result) {
		...
		return env$2.createValue(res, result);
	},
	napi_get_value_bigint_words(env_id, value, sign_bit, word_count, words) {
		let env$2 = environments[env_id];
		let val = env$2.get(value);
		let count = env$2.u32[word_count >> 2];
		if (sign_bit) env$2.i32[sign_bit] = val < 0n ? 1 : 0;
		let i = 0;
		if (words) {
			let mask = (1n << 64n) - 1n;
			let buf = env$2.u64;
		...
		return env$2.setPointer(word_count, i);
	},
	...
	napi_set_element(env_id, object, index, value) {
		let env$2 = environments[env_id];
		let obj = env$2.get(object);
		obj[index] = env$2.get(value);
		return NAPI_OK;
	},
	napi_get_element(env_id, object, index, result) {
		let env$2 = environments[env_id];
		let val = env$2.get(object)[index];
		return env$2.createValue(val, result);
	},
	napi_has_element(env_id, object, index, result) {
		let env$2 = environments[env_id];
		let obj = env$2.get(object);
		env$2.memory[result] = obj.hasOwnProperty(index) ? 1 : 0;
		return NAPI_OK;
	},
	...
	napi_get_array_length(env_id, value, result) {
		let env$2 = environments[env_id];
		let val = env$2.get(value);
		return env$2.setPointer(result, val.length);
	},
	...
	napi_create_function(env_id, utf8name, length, cb, data, result) {
		let env$2 = environments[env_id];
		let func = env$2.createFunction(cb, data);
		Object.defineProperty(func, "name", {
			value: env$2.getString(utf8name, length),
			configurable: true
		});
	...
	napi_call_function(env_id, recv, func, argc, argv, result) {
		let env$2 = environments[env_id];
		let thisArg = env$2.get(recv);
		let fn = env$2.get(func);
		let args = new Array(argc);
		let mem = env$2.u32;
		for (let i = 0; i < argc; i++) {
			args[i] = env$2.get(mem[argv >> 2]);
			argv += 4;
		}
		try {
	...
	napi_new_instance(env_id, cons, argc, argv, result) {
		...
		try {
			...
			return env$2.createValue(value, result);
		} catch (err) {
			env$2.pendingException = err;
			return NAPI_PENDING_EXCEPTION;
		}
	...
	napi_get_cb_info(env_id, cbinfo, argc, argv, thisArg, data) {
		let env$2 = environments[env_id];
		let info = env$2.get(cbinfo);
		env$2.setPointer(argc, info.args.length);
		for (let i = 0; i < info.args.length; i++) {
			env$2.createValue(info.args[i], argv);
			argv += 4;
		}
		env$2.createValue(info.thisArg, thisArg);
		env$2.setPointer(data, info.data);
		return NAPI_OK;
	},
	napi_get_new_target(env_id, cbinfo, result) {
		let env$2 = environments[env_id];
		let info = env$2.get(cbinfo);
		return env$2.createValue(info.newTarget, result);
	},
	napi_create_threadsafe_function(env_id, func, async_resource, async_resource_name, max_queue_size, initial_thread_count, thread_finalize_data, thread_finalize_cb, context, call_js_cb, result) {
		let env$2 = environments[env_id];
		let f = new ThreadSafeFunction(env$2, func ? env$2.get(func) : void 0, call_js_cb ? env$2.table.get(call_js_cb) : void 0, context);
		if (thread_finalize_cb) {
			let cb = env$2.table.get(thread_finalize_cb);
			finalizationRegistry.register(f, new FinalizeRecord(env_id, cb, 0, f.id));
		}
		env$2.setPointer(result, f.id);
		return NAPI_OK;
	},
	...
	napi_create_async_work(env_id, async_resource, async_resource_name, execute, complete, data, result) {
		let env$2 = environments[env_id];
		let w = new AsyncWork(env$2, execute ? env$2.table.get(execute) : void 0, complete ? env$2.table.get(complete) : void 0, data);
		env$2.setPointer(result, w.id);
		return NAPI_OK;
	},
	napi_delete_async_work(env$2, work$1) {
		asyncWork[work$1] = void 0;
		return NAPI_OK;
	},
	napi_queue_async_work(env$2, work$1) {
		queueMicrotask(() => {
			let w = asyncWork[work$1];
			if (w) {
				w.execute(env$2, w.data);
				w.complete(env$2, NAPI_OK, w.data);
			}
		});
		return NAPI_OK;
	},
	...
	napi_throw(env_id, error) {
		let env$2 = environments[env_id];
		env$2.pendingException = env$2.get(error);
		return NAPI_OK;
	},
	napi_throw_error(env_id, code, msg) {
		let env$2 = environments[env_id];
		let err = new Error(env$2.getString(msg));
		err.code = code;
		env$2.pendingException = err;
		return NAPI_OK;
	},
	napi_throw_type_error(env_id, code, msg) {
		let env$2 = environments[env_id];
		let err = new TypeError(env$2.getString(msg));
		err.code = code;
		env$2.pendingException = err;
		return NAPI_OK;
	},
	napi_throw_range_error(env_id, code, msg) {
		let env$2 = environments[env_id];
		let err = new RangeError(env$2.getString(msg));
		err.code = code;
		env$2.pendingException = err;
		return NAPI_OK;
	},
	napi_create_error(env_id, code, msg, result) {
		let env$2 = environments[env_id];
		let err = new Error(env$2.get(msg));
		err.code = env$2.get(code);
		return env$2.createValue(err, result);
	},
	napi_create_type_error(env_id, code, msg, result) {
		let env$2 = environments[env_id];
		let err = new TypeError(env$2.get(msg));
		err.code = env$2.get(code);
		return env$2.createValue(err, result);
	},
	napi_create_range_error(env_id, code, msg, result) {
		let env$2 = environments[env_id];
		let err = new RangeError(env$2.get(msg));
		err.code = env$2.get(code);
		return env$2.createValue(err, result);
	},
	napi_get_and_clear_last_exception(env_id, result) {
		...
		let e = env$2.pendingException;
		env$2.pendingException = null;
		return env$2.createValue(e, result);
	},
	napi_is_exception_pending(env_id, result) {
		let env$2 = environments[env_id];
		env$2.memory[result] = env$2.pendingException ? 1 : 0;
		return NAPI_OK;
	},
	...
	napi_create_buffer(env_id, length, data, result) {
		let env$2 = environments[env_id];
		let ptr = env$2.instance.exports.napi_wasm_malloc(length);
		if (data) env$2.setPointer(data, ptr);
		let buf = typeof globalThis.Buffer !== "undefined" ? globalThis.Buffer.from(env$2.memory.buffer, ptr, length) : env$2.memory.subarray(ptr, ptr + length);
		return env$2.createValue(buf, result);
	},
	napi_create_buffer_copy(env_id, length, data, result_data, result) {
		let env$2 = environments[env_id];
		let ptr = env$2.instance.exports.napi_wasm_malloc(length);
		env$2.memory.set(env$2.memory.subarray(data, data + length), ptr);
		if (result_data) env$2.setPointer(result_data, ptr);
		let res = typeof globalThis.Buffer !== "undefined" ? globalThis.Buffer.from(env$2.memory.buffer, ptr, length) : env$2.memory.subarray(ptr, ptr + length);
		return env$2.createValue(res, result);
	},
	napi_create_external_buffer(env_id, length, data, finalize_cb, finalize_hint, result) {
		let env$2 = environments[env_id];
		let buf = typeof globalThis.Buffer !== "undefined" ? globalThis.Buffer.from(env$2.memory.buffer, data, length) : env$2.memory.subarray(data, data + length);
		if (finalize_cb) {
			let cb = env$2.table.get(finalize_cb);
	...
	napi_get_buffer_info(env_id, value, data, length) {
		let env$2 = environments[env_id];
		let buf = env$2.get(value);
		let len = env$2.getBufferInfo(buf, data);
		return env$2.setPointer(length, len);
	},
	napi_create_arraybuffer(env_id, length, data, result) {
		let env$2 = environments[env_id];
		let buf = new ArrayBuffer(length);
		if (data) env$2.getBufferInfo(buf, data);
		return env$2.createValue(buf, result);
	},
	...
	napi_get_arraybuffer_info(env_id, value, data, length) {
		let env$2 = environments[env_id];
		let len = env$2.getBufferInfo(env$2.get(value), data);
		return env$2.setPointer(length, len);
	},
	...
	napi_is_detached_arraybuffer(env_id, arraybuffer, result) {
		let env$2 = environments[env_id];
		let buffer = env$2.get(arraybuffer);
		env$2.memory[result] = buffer.byteLength === 0 ? 1 : 0;
		return NAPI_OK;
	},
	napi_create_typedarray(env_id, type, length, arraybuffer, offset, result) {
		let env$2 = environments[env_id];
		let Class = typedArrays[type];
		let buf = new Class(env$2.get(arraybuffer), offset, length);
		return env$2.createValue(buf, result);
	},
	napi_create_dataview(env_id, byte_length, arraybuffer, byte_offset, result) {
		...
		return env$2.createValue(view, result);
	},
	napi_get_typedarray_info(env_id, typedarray, type, length, data, arraybuffer, byte_offset) {
		let env$2 = environments[env_id];
		let val = env$2.get(typedarray);
		env$2.setPointer(type, typedArrays.indexOf(val.constructor));
		env$2.setPointer(length, val.length);
		env$2.getBufferInfo(val, data);
		env$2.createValue(val.buffer, arraybuffer);
		return env$2.setPointer(byte_offset, val.byteOffset);
	},
	napi_get_dataview_info(env_id, dataview, byte_length, data, arraybuffer, byte_offset) {
		let env$2 = environments[env_id];
		let val = env$2.get(dataview);
		env$2.setPointer(byte_length, val.byteLength);
		env$2.getBufferInfo(val, data);
		env$2.createValue(val.buffer, arraybuffer);
		return env$2.setPointer(byte_offset, val.byteOffset);
	},
	napi_create_string_utf8(env_id, str, length, result) {
		let env$2 = environments[env_id];
		let s = decoder.decode(env$2.memory.subarray(str, str + length));
		return env$2.createValue(s, result);
	},
	napi_get_value_string_utf8(env_id, value, buf, bufsize, result) {
		let env$2 = environments[env_id];
		let val = env$2.get(value);
		if (buf == 0) return env$2.setPointer(result, utf8Length$1(val));
		let res = encoder$1.encodeInto(val, env$2.memory.subarray(buf, buf + bufsize - 1));
		env$2.memory[buf + res.written] = 0;
		return env$2.setPointer(result, res.written);
	},
	napi_create_string_latin1(env_id, str, length, result) {
		let env$2 = environments[env_id];
		let s = latin1Decoder.decode(env$2.memory.subarray(str, str + length));
		return env$2.createValue(s, result);
	},
	napi_get_value_string_latin1(env_id, value, buf, bufsize, result) {
		let env$2 = environments[env_id];
		let val = env$2.get(value);
		if (buf == 0) return env$2.setPointer(result, val.length);
		let mem = env$2.memory;
		...
		mem[buf] = 0;
		return env$2.setPointer(result, len);
	},
	napi_create_string_utf16(env_id, str, length, result) {
		let env$2 = environments[env_id];
		let s = utf16Decoder.decode(env$2.memory.subarray(str, str + length * 2));
		return env$2.createValue(s, result);
	},
	...
	napi_get_date_value(env_id, value, result) {
		let env$2 = environments[env_id];
		let date = env$2.get(value);
		env$2.f64[result >> 3] = date.valueOf();
	},
	napi_create_symbol(env_id, description, result) {
		let env$2 = environments[env_id];
		let desc = env$2.get(description);
		return env$2.createValue(Symbol(desc), result);
	},
	napi_coerce_to_bool(env_id, value, result) {
		let env$2 = environments[env_id];
		return env$2.createValue(Boolean(env$2.get(value)), result);
	},
	napi_coerce_to_number(env_id, value, result) {
		let env$2 = environments[env_id];
		return env$2.createValue(Number(env$2.get(value)), result);
	},
	napi_coerce_to_object(env_id, value, result) {
		let env$2 = environments[env_id];
		return env$2.createValue(Object(env$2.get(value)), result);
	},
	napi_coerce_to_string(env_id, value, result) {
		let env$2 = environments[env_id];
		return env$2.createValue(String(env$2.get(value)), result);
	},
	napi_typeof(env_id, value, result) {
		let env$2 = environments[env_id];
		let val = env$2.get(value);
		return env$2.setPointer(result, (() => {
			switch (typeof val) {
	...
	napi_instanceof(env_id, object, constructor, result) {
		let env$2 = environments[env_id];
		let obj = env$2.get(object);
		let cons = env$2.get(constructor);
		env$2.memory[result] = obj instanceof cons ? 1 : 0;
		return NAPI_OK;
	},
	napi_is_array(env_id, value, result) {
		let env$2 = environments[env_id];
		let val = env$2.get(value);
		env$2.memory[result] = Array.isArray(val) ? 1 : 0;
		return NAPI_OK;
	},
	napi_is_buffer(env_id, value, result) {
		let env$2 = environments[env_id];
		let val = env$2.get(value);
		env$2.memory[result] = (typeof globalThis.Buffer !== "undefined" ? globalThis.Buffer.isBuffer(val) : val instanceof Uint8Array) ? 1 : 0;
		return NAPI_OK;
	},
	napi_is_date(env_id, value, result) {
		let env$2 = environments[env_id];
		let val = env$2.get(value);
		env$2.memory[result] = val instanceof Date ? 1 : 0;
		return NAPI_OK;
	},
	napi_is_error(env_id, value, result) {
		let env$2 = environments[env_id];
		let err = env$2.get(value);
		env$2.memory[result] = err instanceof Error ? 1 : 0;
		return NAPI_OK;
	},
	napi_is_typedarray(env_id, value, result) {
		let env$2 = environments[env_id];
		let buf = env$2.get(value);
		env$2.memory[result] = ArrayBuffer.isView(buf) && !(buf instanceof DataView) ? 1 : 0;
		return NAPI_OK;
	},
	napi_is_dataview(env_id, value, result) {
		let env$2 = environments[env_id];
		let val = env$2.get(value);
		env$2.memory[result] = val instanceof DataView ? 1 : 0;
		return NAPI_OK;
	},
	napi_strict_equals(env_id, lhs, rhs, result) {
		let env$2 = environments[env_id];
		env$2.memory[result] = env$2.get(lhs) === env$2.get(rhs) ? 1 : 0;
		return NAPI_OK;
	},
	napi_wrap(env_id, js_object, native_object, finalize_cb, finalize_hint, result) {
		let env$2 = environments[env_id];
		let obj = env$2.get(js_object);
		env$2.wrappedObjects.set(obj, native_object);
		if (finalize_cb) {
			let cb = env$2.table.get(finalize_cb);
	...
	napi_unwrap(env_id, js_object, result) {
		let env$2 = environments[env_id];
		let obj = env$2.get(js_object);
		let native_object = env$2.wrappedObjects.get(obj);
		env$2.setPointer(result, native_object);
		return NAPI_OK;
	},
	napi_remove_wrap(env_id, js_object, result) {
		...
		env$2.wrappedObjects.delete(obj);
		return env$2.setPointer(result, native_object);
	},
	...
	napi_create_promise(env_id, deferred, promise) {
		...
		let p = new Promise((resolve, reject) => {
			let id = env$2.deferred.length;
			env$2.deferred.push({
				resolve,
				reject
			});
			env$2.setPointer(deferred, id);
		});
		return env$2.createValue(p, promise);
	},
	napi_resolve_deferred(env_id, deferred, resolution) {
		let env$2 = environments[env_id];
		let { resolve } = env$2.deferred[deferred];
		resolve(env$2.get(resolution));
		env$2.deferred[deferred] = void 0;
		return NAPI_OK;
	},
	napi_reject_deferred(env_id, deferred, rejection) {
		let env$2 = environments[env_id];
		let { reject } = env$2.deferred[deferred];
		reject(env$2.get(rejection));
		env$2.deferred[deferred] = void 0;
		return NAPI_OK;
	},
	napi_is_promise(env_id, value, result) {
		let env$2 = environments[env_id];
		let val = env$2.get(value);
		env$2.memory[result] = val instanceof Promise ? 1 : 0;
		return NAPI_OK;
	},
	napi_run_script(env_id, script, result) {
		let env$2 = environments[env_id];
		let source = env$2.get(script);
	...
	napi_create_external(env_id, data, finalize_cb, finalize_hint, result) {
		...
		env$2.externalObjects.set(external, data);
		if (finalize_cb) {
			let cb = env$2.table.get(finalize_cb);
			finalizationRegistry.register(external, new FinalizeRecord(env_id, cb, finalize_hint, data));
		}
		return env$2.createValue(external, result);
	},
	napi_get_value_external(env_id, value, result) {
		let env$2 = environments[env_id];
		let external = env$2.get(value);
		let val = env$2.externalObjects.get(external);
		return env$2.setPointer(result, val);
	},
...
//#endregion
//#region ../../node_modules/.pnpm/@parcel+watcher-wasm@2.5.1/node_modules/@parcel/watcher-wasm/wrapper.js
...
//#endregion
//#region ../../node_modules/.pnpm/@parcel+watcher-wasm@2.5.1/node_modules/@parcel/watcher-wasm/index.mjs
...
const watches = [null];
const wasm_env = {
	__syscall_newfstatat(dirfd, path$1, buf, flags) {
		let dir = dirfd === -100 ? process.cwd() : fds.get(dirfd).path;
		let p = Path.resolve(dir, env$1.getString(path$1));
	...
	__syscall_lstat64(path$1, buf) {
		let p = env$1.getString(path$1);
		try {
	...
	__syscall_openat(dirfd, path$1, flags, mode) {
		let f = 0;
	...
	wasm_backend_add_watch(filename, backend) {
		let path$1 = env$1.getString(filename);
		let watch = fs.watch(path$1, { encoding: "buffer" }, (eventType, filename$1) => {
			if (filename$1) {
				let type = eventType === "change" ? 1 : 2;
				let fptr = env$1.instance.exports.malloc(filename$1.byteLength + 1);
				env$1.memory.set(filename$1, fptr);
				env$1.memory[fptr + filename$1.byteLength] = 0;
