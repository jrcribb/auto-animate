...
import { d as Wt, n as logger, u as Tt } from "./logger-O9Su9xI5.mjs";
...
const command = defineCommand({
	...
	async run(ctx) {
		...
		const { loadNuxt } = await loadKit(cwd$1);
				...
				overrides: {
					...
					modules: [function(_, nuxt) {
						nuxt.hook("nitro:init", (nitro) => {
							res(resolve$1(nuxt.options.srcDir || cwd$1, nitro.options.output.dir || ".output", "nitro.json"));
						});
					}]
				}
			}).then((nuxt) => nuxt.close()).catch(() => "");
		}), resolve$1(cwd$1, ".output", "nitro.json")].filter(Boolean);
		...
		if (existsSync(resolve$1(cwd$1, envFileName))) {
