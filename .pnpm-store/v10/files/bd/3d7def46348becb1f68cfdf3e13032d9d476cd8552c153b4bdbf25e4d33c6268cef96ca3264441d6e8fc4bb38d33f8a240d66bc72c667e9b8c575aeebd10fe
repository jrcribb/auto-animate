import { i as __require, n as __esmMin, t as __commonJSMin } from "./chunk-CuftcezF.mjs";
import { n as colors } from "./dist-0DfurPla.mjs";
import "./utils-D5jIbZ8_.mjs";
import { t as defineCommand } from "./dist-BBpbTs93.mjs";
import { i as c, o as v, t as D } from "./dist-DS-XJpoP.mjs";
import { r as version } from "./package-CnpCwgZx.mjs";
import { a as legacyRootDirArgs, t as cwdArgs } from "./_shared-CThtxn2Y.mjs";
import { n as logger, u as Tt } from "./logger-O9Su9xI5.mjs";
import { n as tryResolveNuxt } from "./kit-mAWtuYKF.mjs";
import { c as resolve$1 } from "./pathe.M-eThtNZ-C6qzirxH.mjs";
import { r as readPackageJSON } from "./dist-z8aedaRt.mjs";
import "./satisfies-C79zCZRN.mjs";
import "./versions-BUwGlDsD.mjs";
import { t as getBuilder } from "./banner-Djkyn06y.mjs";
import "./main-BgvXMeoL.mjs";
import { r as detectPackageManager } from "./nypm.CLjaS_sz-CuB4zfXi.mjs";
import { t as formatInfoBox } from "./formatting-DDsi7H9i.mjs";
...
//#endregion
//#region ../../node_modules/.pnpm/tsdown@0.17.1_oxc-resolver@11.15.0_synckit@0.11.11_typescript@5.9.3/node_modules/tsdown/esm-shims.js
...
var require_utf16 = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	Utf16BEEncoder.prototype.write = function(str) {
		var buf = Buffer$7.from(str, "ucs2");
	...
	Utf16BEDecoder.prototype.write = function(buf) {
		...
		var buf2 = Buffer$7.alloc(buf.length + 1), i$3 = 0, j = 0;
	...
	Utf16Decoder.prototype.write = function(buf) {
		if (!this.decoder) {
			...
			var buf = Buffer$7.concat(this.initialBytes), encoding = detectEncoding(buf, this.options.defaultEncoding);
	...
	function detectEncoding(buf, defaultEncoding) {
		...
		if (buf.length >= 2) if (buf[0] == 254 && buf[1] == 255) enc$1 = "utf-16be";
			...
			for (var i$3 = 0; i$3 < _len; i$3 += 2) {
				if (buf[i$3] === 0 && buf[i$3 + 1] !== 0) asciiCharsBE++;
				if (buf[i$3] !== 0 && buf[i$3 + 1] === 0) asciiCharsLE++;
			}
...
var require_utf7 = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	Utf7Decoder.prototype.write = function(buf) {
		...
		for (var i$3 = 0; i$3 < buf.length; i$3++) if (!inBase64) {
			...
			if (i$3 == lastI && buf[i$3] == minusChar) res += "+";
				...
				var b64str = base64Accum + buf.slice(lastI, i$3).toString();
	...
	Utf7Decoder.prototype.end = function() {
		var res = "";
		if (this.inBase64 && this.base64Accum.length > 0) res = this.iconv.decode(Buffer$6.from(this.base64Accum, "base64"), "utf16-be");
		this.inBase64 = false;
		this.base64Accum = "";
		return res;
	};
	...
	Utf7IMAPEncoder.prototype.write = function(str) {
		var inBase64 = this.inBase64, base64Accum = this.base64Accum, base64AccumIdx = this.base64AccumIdx, buf = Buffer$6.alloc(str.length * 5 + 10), bufIdx = 0;
		for (var i$3 = 0; i$3 < str.length; i$3++) {
	...
	Utf7IMAPDecoder.prototype.write = function(buf) {
		...
		for (var i$3 = 0; i$3 < buf.length; i$3++) if (!inBase64) {
			...
			if (i$3 == lastI && buf[i$3] == minusChar) res += "&";
				...
				var b64str = base64Accum + buf.slice(lastI, i$3).toString().replace(/,/g, "/");
				res += this.iconv.decode(Buffer$6.from(b64str, "base64"), "utf16-be");
			}
			if (buf[i$3] != minusChar) i$3--;
			lastI = i$3 + 1;
...
var require_sbcs_codec = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	function SBCSCodec(codecOptions, iconv$2) {
		...
		var encodeBuf = Buffer$5.alloc(65536, iconv$2.defaultCharSingleByte.charCodeAt(0));
...
var require_dbcs_codec = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	function DBCSCodec(codecOptions, iconv$2) {
		...
		if (typeof codecOptions.gb18030 === "function") {
			...
			for (var i$3 = 129; i$3 <= 254; i$3++) {
				var secondByteNodeIdx = NODE_START - this.decodeTables[0][i$3];
				var secondByteNode = this.decodeTables[secondByteNodeIdx];
				for (var j = 48; j <= 57; j++) secondByteNode[j] = NODE_START - thirdByteNodeIdx;
			}
			for (var i$3 = 129; i$3 <= 254; i$3++) thirdByteNode[i$3] = NODE_START - fourthByteNodeIdx;
			for (var i$3 = 48; i$3 <= 57; i$3++) fourthByteNode[i$3] = GB18030_CODE;
		}
	}
	...
	DBCSCodec.prototype._getDecodeTrieNode = function(addr) {
		...
		for (var i$3 = bytes.length - 1; i$3 > 0; i$3--) {
			var val = node[bytes[i$3]];
			if (val == UNASSIGNED) {
				node[bytes[i$3]] = NODE_START - this.decodeTables.length;
	...
	DBCSEncoder.prototype.write = function(str) {
		var newBuf = Buffer$4.alloc(str.length * (this.gb18030 ? 4 : 3)), leadSurrogate = this.leadSurrogate, seqObj = this.seqObj, nextChar = -1, i$3 = 0, j = 0;
		while (true) {
			if (nextChar === -1) {
	...
	DBCSEncoder.prototype.end = function() {
		...
		var newBuf = Buffer$4.alloc(10), j = 0;
		if (this.seqObj) {
	...
	DBCSDecoder.prototype.write = function(buf) {
		var newBuf = Buffer$4.alloc(buf.length * 2), nodeIdx = this.nodeIdx, prevBuf = this.prevBuf, prevBufOffset = this.prevBuf.length, seqStart = -this.prevBuf.length, uCode;
		if (prevBufOffset > 0) prevBuf = Buffer$4.concat([prevBuf, buf.slice(0, 10)]);
		for (var i$3 = 0, j = 0; i$3 < buf.length; i$3++) {
			var curByte = i$3 >= 0 ? buf[i$3] : prevBuf[i$3 + prevBufOffset];
			var uCode = this.decodeTables[nodeIdx][curByte];
			if (uCode >= 0) {} else if (uCode === UNASSIGNED) {
				i$3 = seqStart;
				uCode = this.defaultCharUnicode.charCodeAt(0);
			} else if (uCode === GB18030_CODE) {
				var curSeq = seqStart >= 0 ? buf.slice(seqStart, i$3 + 1) : prevBuf.slice(seqStart + prevBufOffset, i$3 + 1 + prevBufOffset);
...
var require_streams = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var Buffer$3 = __require("buffer").Buffer, Transform = __require("stream").Transform;
	module.exports = function(iconv$2) {
		iconv$2.encodeStream = function encodeStream(encoding, options) {
			return new IconvLiteEncoderStream(iconv$2.getEncoder(encoding, options), options);
		};
		iconv$2.decodeStream = function decodeStream(encoding, options) {
			return new IconvLiteDecoderStream(iconv$2.getDecoder(encoding, options), options);
		};
		iconv$2.supportsStreams = true;
		iconv$2.IconvLiteEncoderStream = IconvLiteEncoderStream;
		iconv$2.IconvLiteDecoderStream = IconvLiteDecoderStream;
		iconv$2._collect = IconvLiteDecoderStream.prototype.collect;
	};
...
var require_extend_node = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	module.exports = function(iconv$2) {
		var original = void 0;
		iconv$2.supportsNodeEncodingsExtension = !(Buffer$2.from || new Buffer$2(0) instanceof Uint8Array);
		iconv$2.extendNodeEncodings = function extendNodeEncodings() {
			if (original) return;
			original = {};
			if (!iconv$2.supportsNodeEncodingsExtension) {
			...
			Buffer$2.isNativeEncoding = function(enc$1) {
			...
			SlowBuffer.prototype.toString = function(encoding, start, end) {
				...
				return iconv$2.decode(this.slice(start, end), encoding);
			};
			...
			SlowBuffer.prototype.write = function(string, offset, length, encoding) {
				...
				var buf = iconv$2.encode(string, encoding);
			...
			Buffer$2.isEncoding = function(encoding) {
				return Buffer$2.isNativeEncoding(encoding) || iconv$2.encodingExists(encoding);
			};
			...
			Buffer$2.byteLength = SlowBuffer.byteLength = function(str, encoding) {
				...
				return iconv$2.encode(str, encoding).length;
			};
			...
			if (iconv$2.supportsStreams) {
				var Readable = __require("stream").Readable;
				original.ReadableSetEncoding = Readable.prototype.setEncoding;
				Readable.prototype.setEncoding = function setEncoding(enc$1, options) {
					this._readableState.decoder = iconv$2.getDecoder(enc$1, options);
					this._readableState.encoding = enc$1;
				};
				Readable.prototype.collect = iconv$2._collect;
			}
		};
		iconv$2.undoExtendNodeEncodings = function undoExtendNodeEncodings() {
			if (!iconv$2.supportsNodeEncodingsExtension) return;
...
var require_lib = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	iconv$1.getDecoder = function getDecoder(encoding, options) {
	...
	if (nodeVer) {
		...
		if (nodeVerArr[0] > 0 || nodeVerArr[1] >= 10) require_streams()(iconv$1);
		require_extend_node()(iconv$1);
	}
}));
...
var require_copy_paste = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	exports.copy = (text, callback) => {
		...
		const err = [];
		child.stdin.on("error", (err$1) => done(err$1));
		child.on("exit", () => done(null, text)).on("error", (err$1) => done(err$1)).stderr.on("data", (chunk) => err.push(chunk)).on("end", () => {
	...
	exports.paste = (callback) => {
		...
		const data = [];
		const err = [];
		child.on("error", (err$1) => done(err$1));
...
var info_default = defineCommand({
	...
	async run(ctx) {
		const cwd$1 = resolve$1(ctx.args.cwd || ctx.args.rootDir);
		const nuxtConfig = await getNuxtConfig(cwd$1);
		const { dependencies = {}, devDependencies = {} } = await readPackageJSON(cwd$1).catch(() => ({}));
		const nuxtPath = tryResolveNuxt(cwd$1);
		async function getDepVersion(name) {
			for (const url of [cwd$1, nuxtPath]) {
				if (!url) continue;
		...
		async function listModules(arr = []) {
			...
			for (let m of arr) {
				...
				const name = normalizeConfigModule(m, cwd$1);
				if (name) {
					const v$1 = await getDepVersion(name.split("/").splice(0, 2).join("/"));
					info.push(`\`${v$1 ? `${name}@${v$1}` : name}\``);
				}
			}
			return info.join(", ");
		}
		...
		let packageManager = (await detectPackageManager(cwd$1))?.name;
		if (packageManager) packageManager += `@${getPackageManagerVersion(packageManager)}`;
		const osType = os.type();
		const builderInfo = typeof builder === "string" ? getBuilder(cwd$1, builder) : {
			name: "custom",
			version: "0.0.0"
		};
		const infoObj = {
			...
			"Config": Object.keys(nuxtConfig).map((key$1) => `\`${key$1}\``).sort().join(", "),
			"Modules": await listModules(nuxtConfig.modules),
			...isLegacy ? { "Build modules": await listModules(nuxtConfig.buildModules || []) } : {}
		};
		logger.info(`Nuxt root directory: ${colors.cyan(nuxtConfig.rootDir || cwd$1)}\n`);
		...
		const copied = !v && await new Promise((resolve$2) => (0, import_copy_paste.copy)(copyStr, (err) => resolve$2(!err)));
...
function normalizeConfigModule(module$2, rootDir) {
	if (!module$2) return null;
	if (typeof module$2 === "string") return module$2.split(rootDir).pop().split("node_modules").pop().replace(/^\//, "");
	if (typeof module$2 === "function") return `${module$2.name}()`;
	if (Array.isArray(module$2)) return normalizeConfigModule(module$2[0], rootDir);
	return null;
}
async function getNuxtConfig(rootDir) {
	try {
		const { createJiti } = await import("./jiti-DpOivgAp.mjs");
		...
		globalThis.defineNuxtConfig = (c$1) => c$1;
