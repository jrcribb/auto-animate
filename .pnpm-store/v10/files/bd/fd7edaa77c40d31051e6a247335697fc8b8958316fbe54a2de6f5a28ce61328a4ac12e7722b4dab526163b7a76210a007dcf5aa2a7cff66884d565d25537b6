import { s as __toESM$1, t as __commonJSMin } from "./chunk-CuftcezF.mjs";
...
var require_lib = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	function isValidAmbientConstInitializer(expression, estree$1) {
		...
		if (isNumber$1(expression, estree$1) || isNegativeNumber(expression, estree$1)) return true;
		if (type === "TemplateLiteral" && expression.expressions.length === 0) return true;
		if (isPossiblyLiteralEnum(expression)) return true;
		return false;
	}
	function isNumber$1(expression, estree$1) {
	...
	function isNegativeNumber(expression, estree$1) {
		if (expression.type === "UnaryExpression") {
			...
			if (operator === "-" && isNumber$1(argument, estree$1)) return true;
		}
		return false;
	}
...
function shared_default(fork) {
	var types$1 = fork.use(typesPlugin);
	var Type$1 = types$1.Type;
	var builtin = types$1.builtInTypes;
	var isNumber$1$1 = builtin.number;
	function geq(than) {
		return Type$1.from((value) => isNumber$1$1.check(value) && value >= than, isNumber$1$1 + " >= " + than);
	}
	const defaults$1 = {
		"null": function() {
			return null;
...
function typesPlugin(_fork) {
	const Type$1 = {
		or(...types$1) {
			return new OrType(types$1.map((type) => Type$1.from(type)));
		},
		from(value, name) {
	...
	const isNumber$1$1 = defBuiltInType("number", 3);
...
function pathPlugin(fork) {
	var types$1 = fork.use(typesPlugin);
	var isArray$4 = types$1.builtInTypes.array;
	var isNumber$1$1 = types$1.builtInTypes.number;
	...
	function getMoves(path, offset, start, end) {
		...
		isNumber$1$1.assert(start);
		isNumber$1$1.assert(end);
...
function scopePlugin(fork) {
	var types$1 = fork.use(typesPlugin);
	var Type$1 = types$1.Type;
	var namedTypes$2 = types$1.namedTypes;
	var Node$1 = namedTypes$2.Node;
	var Expression$1 = namedTypes$2.Expression;
	var isArray$4 = types$1.builtInTypes.array;
	var b$7 = types$1.builders;
	const Scope$1 = function Scope$1$1(path, parentScope) {
		if (!(this instanceof Scope$1$1)) throw new Error("Scope constructor cannot be invoked without 'new'");
		if (!TypeParameterScopeType.check(path.value)) ScopeType.assert(path.value);
		var depth;
		if (parentScope) {
			if (!(parentScope instanceof Scope$1$1)) throw new Error("");
	...
	Scope$1.isEstablishedBy = function(node) {
		return ScopeType.check(node) || TypeParameterScopeType.check(node);
	};
	var Sp = Scope$1.prototype;
	Sp.didScan = false;
	...
	Sp.declareTemporary = function(prefix$1) {
		if (prefix$1) {
			if (!/^[a-z$_]/i.test(prefix$1)) throw new Error("");
		} else prefix$1 = "t$";
		prefix$1 += this.depth.toString(36) + "$";
		this.scan();
		var index = 0;
		while (this.declares(prefix$1 + index)) ++index;
		var name = prefix$1 + index;
		return this.bindings[name] = types$1.builders.identifier(name);
	};
	...
	function recursiveScanScope(path, bindings, scopeTypes) {
		...
		else if (Node$1.check(node) && !Expression$1.check(node)) types$1.eachField(node, function(name, child) {
	...
	function addTypePattern(patternPath, types$1$1) {
		var pattern = patternPath.value;
		namedTypes$2.Pattern.assert(pattern);
		if (namedTypes$2.Identifier.check(pattern)) if (hasOwn$4.call(types$1$1, pattern.name)) types$1$1[pattern.name].push(patternPath);
		else types$1$1[pattern.name] = [patternPath];
	}
	function addTypeParameter(parameterPath, types$1$1) {
		var parameter = parameterPath.value;
		FlowOrTSTypeParameterType.assert(parameter);
		if (hasOwn$4.call(types$1$1, parameter.name)) types$1$1[parameter.name].push(parameterPath);
		else types$1$1[parameter.name] = [parameterPath];
	}
...
function nodePathPlugin(fork) {
	var types$1 = fork.use(typesPlugin);
	var n$4 = types$1.namedTypes;
	var b$7 = types$1.builders;
	var isNumber$1$1 = types$1.builtInTypes.number;
	var isArray$4 = types$1.builtInTypes.array;
	var Path$1 = fork.use(pathPlugin);
	var Scope$1 = fork.use(scopePlugin);
	...
	NPp._computeScope = function() {
		...
		if (n$4.Node.check(value) && Scope$1.isEstablishedBy(value)) scope = new Scope$1(this, scope);
		return scope || null;
	};
	NPp.getValueProperty = function(name) {
		return types$1.getFieldValue(this.value, name);
	};
	/**
	...
	*/
	NPp.needsParens = function(assumeExpressionContext) {
		...
		switch (node.type) {
			...
			case "Literal": return parent.type === "MemberExpression" && isNumber$1$1.check(node.value) && this.name === "object" && parent.object === node;
			case "AssignmentExpression":
	...
	function containsCallExpression$1(node) {
		...
		if (n$4.Node.check(node)) return types$1.someField(node, function(_name, child) {
...
function pathVisitorPlugin(fork) {
	...
	var isArray$4 = types$1.builtInTypes.array;
	var isObject$4 = types$1.builtInTypes.object;
	var isFunction = types$1.builtInTypes.function;
	var undefined$1;
	...
	function computeMethodNameTable(visitor) {
		...
		var supertypeTable = types$1.computeSupertypeLookupTable(typeNames);
	...
	function visitChildren(path, visitor) {
		...
		if (isArray$4.check(value)) path.each(visitor.visitWithoutReset, visitor);
			...
			var childNames = types$1.getFieldNames(value);
			if (visitor._shouldVisitComments && value.comments && childNames.indexOf("comments") < 0) childNames.push("comments");
			var childCount = childNames.length;
			var childPaths = [];
			for (var i = 0; i < childCount; ++i) {
				var childName = childNames[i];
				if (!hasOwn$3.call(value, childName)) value[childName] = types$1.getFieldValue(value, childName);
...
function equiv_default(fork) {
	var types$1 = fork.use(typesPlugin);
	var getFieldNames$1 = types$1.getFieldNames;
	var getFieldValue$1 = types$1.getFieldValue;
	var isArray$4 = types$1.builtInTypes.array;
	var isObject$4 = types$1.builtInTypes.object;
	var isDate = types$1.builtInTypes.Date;
	var isRegExp = types$1.builtInTypes.RegExp;
...
function fork_default(plugins) {
	...
	const types$1 = fork.use(typesPlugin);
	plugins.forEach(fork.use);
	types$1.finalize();
	const PathVisitor$1 = fork.use(pathVisitorPlugin);
	return {
		Type: types$1.Type,
		builtInTypes: types$1.builtInTypes,
		namedTypes: types$1.namedTypes,
		builders: types$1.builders,
		defineMethod: types$1.defineMethod,
		getFieldNames: types$1.getFieldNames,
		getFieldValue: types$1.getFieldValue,
		eachField: types$1.eachField,
		someField: types$1.someField,
		getSupertypeNames: types$1.getSupertypeNames,
		getBuilderName: types$1.getBuilderName,
		astNodesAreEquivalent: fork.use(equiv_default),
		finalize: types$1.finalize,
...
function es6_default(fork) {
	...
	const types$1 = fork.use(typesPlugin);
	const def = types$1.Type.def;
	const or = types$1.Type.or;
...
function type_annotations_default(fork) {
	var types$1 = fork.use(typesPlugin);
	var def = types$1.Type.def;
	var or = types$1.Type.or;
...
function babel_core_default(fork) {
	...
	const def = types$1.Type.def;
	const or = types$1.Type.or;
	const { undefined: isUndefined } = types$1.builtInTypes;
	...
	function makeLiteralExtra(rawValueType = String, toRaw) {
		return [
			...
			function getDefault() {
				const value = types$1.getFieldValue(this, "value");
				return {
					rawValue: value,
...
function typescript_default(fork) {
	...
	var or = types$1.Type.or;
	var defaults$1 = fork.use(shared_default).defaults;
	var StringLiteral = types$1.Type.from(function(value, deep) {
...
const PrintResult = function PrintResult$1(code$1, sourceMap$2) {
...
function proxifyModule(ast, code$1) {
...
function detectCodeFormat(code$1, userStyles = {}) {
	const detect = {
...
function parseModule(code$1, options) {
	return proxifyModule(parse(code$1, {
...
function generateCode(node, options = {}) {
	...
	const { code: code$1, map } = print(ast, {
		...options,
		...formatOptions
	});
	return {
