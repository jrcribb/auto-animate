...
import { i as c, r as a, t as D } from "./dist-DS-XJpoP.mjs";
...
import { a as require_lib, i as listenAndWatch, n as createDevServer, o as require_http_shutdown, r as listen, t as initialize } from "./dev-DLPJ5wS3.mjs";
import { n as isAbsolute$1 } from "./pathe.ff20891b-B8oofkvE.mjs";
...
var import_http_shutdown = require_http_shutdown();
...
var ForkPool = class {
	...
	createFork() {
		const childProc = fork(globalThis.__nuxt_cli__.devEntry, this.rawArgs, {
			execArgv: ["--enable-source-maps", process.argv.find((a$1) => a$1.includes("--inspect"))].filter(Boolean),
			env: {
				...process.env,
				__NUXT__FORK: "true"
			}
		...
		const pooledFork = {
			process: childProc,
			ready: new Promise((resolve$2, reject) => {
				readyResolve = resolve$2;
				readyReject = reject;
			}),
			state: "warming"
		};
	...
	killFork(fork$1, signal = "SIGTERM") {
		fork$1.state = "dead";
		if (fork$1.process) fork$1.process.kill(signal === 0 && D ? "SIGTERM" : signal);
		this.removeFork(fork$1);
	}
	removeFork(fork$1) {
