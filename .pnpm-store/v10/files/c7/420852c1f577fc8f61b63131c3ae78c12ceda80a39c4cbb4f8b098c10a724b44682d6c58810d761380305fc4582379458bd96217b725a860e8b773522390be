...
var require_range = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	var Range$2 = class Range$2 {
		constructor(range, options) {
			...
			if (range instanceof Range$2) if (range.loose === !!options.loose && range.includePrerelease === !!options.includePrerelease) return range;
			else return new Range$2(range.raw, options);
		...
		intersects(range, options) {
			if (!(range instanceof Range$2)) throw new TypeError("a Range is required");
	...
	const testSet = (set, version, options) => {
		...
		if (version.prerelease.length && !options.includePrerelease) {
			for (let i = 0; i < set.length; i++) {
				...
				if (set[i].semver === Comparator$1.ANY) continue;
