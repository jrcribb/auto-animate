import { i as __require, s as __toESM, t as __commonJSMin } from "./chunk-CuftcezF.mjs";
...
var require_windows = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	function checkStat$1(stat, path$4, options) {
		if (!stat.isSymbolicLink() && !stat.isFile()) return false;
		return checkPathExt(path$4, options);
	}
	function isexe$3(path$4, options, cb) {
		fs$3.stat(path$4, function(er, stat) {
			cb(er, er ? false : checkStat$1(stat, path$4, options));
		});
	}
	function sync$2(path$4, options) {
		return checkStat$1(fs$3.statSync(path$4), path$4, options);
	}
}));
...
var require_mode = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	function isexe$2(path$4, options, cb) {
		fs$2.stat(path$4, function(er, stat) {
			cb(er, er ? false : checkStat(stat, options));
		});
	}
	function sync$1(path$4, options) {
...
var require_isexe = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	isexe$1.sync = sync;
	function isexe$1(path$4, options, cb) {
		...
		if (!cb) {
			...
			return new Promise(function(resolve$1, reject) {
				isexe$1(path$4, options || {}, function(er, is) {
					if (er) reject(er);
					else resolve$1(is);
				});
			});
...
var require_which = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	const which$1 = (cmd, opt, cb) => {
		...
		const found = [];
		const step = (i) => new Promise((resolve$1, reject) => {
			if (i === pathEnv.length) return opt.all && found.length ? resolve$1(found) : reject(getNotFoundError(cmd));
			const ppRaw = pathEnv[i];
			const pathPart = /^".*"$/.test(ppRaw) ? ppRaw.slice(1, -1) : ppRaw;
			const pCmd = path$3.join(pathPart, cmd);
			resolve$1(subStep(!pathPart && /^\.[\\\/]/.test(cmd) ? cmd.slice(0, 2) + pCmd : pCmd, i, 0));
		});
		const subStep = (p, i, ii) => new Promise((resolve$1, reject) => {
			if (ii === pathExt.length) return resolve$1(step(i + 1));
			const ext = pathExt[ii];
			isexe(p + ext, { pathExt: pathExtExe }, (er, is) => {
				if (!er && is) if (opt.all) found.push(p + ext);
				else return resolve$1(p + ext);
				return resolve$1(subStep(p, i, ii + 1));
			});
		});
...
function pathKey(options = {}) {
	const { env: env$1 = process.env, platform = process.platform } = options;
...
const npmRunPath = ({ cwd: cwd$1 = process$1.cwd(), path: pathOption = process$1.env[pathKey()], preferLocal = true, execPath = process$1.execPath, addExecPath = true } = {}) => {
	const cwdString = cwd$1 instanceof URL ? fileURLToPath(cwd$1) : cwd$1;
...
const findSignalByNumber = (number, signals$1) => {
	const signal = signals$1.find(({ name }) => constants$1.signals[name] === number);
...
const makeError$1 = ({ stdout: stdout$1, stderr, all, error, signal, exitCode, command, escapedCommand, timedOut, isCanceled, killed, parsed: { options: { timeout, cwd: cwd$1 = process$1.cwd() } } }) => {
...
const setupTimeout = (spawned, { timeout, killSignal = "SIGTERM" }, spawnedPromise) => {
	...
	let timeoutId;
	const timeoutPromise = new Promise((resolve$1, reject) => {
...
const getSpawnedResult = async ({ stdout: stdout$1, stderr, all }, { encoding, buffer, maxBuffer }, processDone) => {
	const stdoutPromise = getStreamPromise(stdout$1, {
		encoding,
		buffer,
		maxBuffer
	});
	...
	try {
		...
		return Promise.all([
			...
			getBufferedData(stdout$1, stdoutPromise),
...
const getSpawnedPromise = (spawned) => new Promise((resolve$1, reject) => {
...
const parseTemplate = ({ templates, expressions, tokens, index, template }) => {
	...
	return concatTokens(newTokens, Array.isArray(expression) ? expression.map((expression$1) => parseExpression(expression$1)) : [parseExpression(expression)], templateString.endsWith(" "));
};
...
function execa(file, args, options) {
	...
	const handlePromise = async () => {
		...
		const stdout$1 = handleOutput(parsed.options, stdoutResult);
...
function execaSync(file, args, options) {
	...
	const stdout$1 = handleOutput(parsed.options, result.stdout, result.error);
...
function create$(options) {
	function $$1(templatesOrOptions, ...expressions) {
...
const clipboard$4 = {
	...
	async paste(options) {
		try {
			const { stdout: stdout$1 } = await execa("termux-clipboard-get", options);
			return stdout$1;
		} catch (error) {
			handler(error);
		}
...
const xselWithFallback = async (argumentList, options) => {
	try {
		const { stdout: stdout$1 } = await execa(xsel, argumentList, options);
		return stdout$1;
	} catch (xselError) {
		try {
			const { stdout: stdout$1 } = await execa(xselFallback, argumentList, options);
			return stdout$1;
		} catch (fallbackError) {
...
const clipboard$2 = {
	...
	async paste(options) {
		const { stdout: stdout$1 } = await execa("pbpaste", {
			...options,
			env
		});
		return stdout$1;
...
function systemArchitectureSync() {
	const { arch, platform, env: env$1 } = process$1;
	...
	if (platform === "win32" && Object.hasOwn(env$1, "PROCESSOR_ARCHITEW6432")) return "x64";
	if (platform === "linux") {
...
const clipboard$1 = {
	...
	async paste(options) {
		const { stdout: stdout$1 } = await execa(windowBinaryPath, ["--paste"], options);
		return stdout$1;
	},
